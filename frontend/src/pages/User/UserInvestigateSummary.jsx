...
import InvestigateHeader from '../Investigate/investigate-header'
import { isAllowedUserSource } from './sourceFilter'
...
export default function UserInvestigateSummary() {
  ...
  const loadAlertBundle = async (id) => {
    ...
    try {
      ...
      const nextBundle = response?.data || null
      if (!isAllowedUserSource(nextBundle?.alert?.source)) {
        setBundle(null)
        setError('Authentication:User can access only source "Autolog" tenant data.')
        return
      }
  ...
  const socRecommendationContent = useMemo(() => {
    const socRecommendation = recommendations.find(
      (recommendation) => String(recommendation?.type || '').toLowerCase() === 'customer'
    )
  ...
  return (
    ...
    >
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '550px 1fr 10px', gap: 16, alignItems: 'start' }}>
        ...
        <main style={{ display: 'grid', gap: 16, minWidth: 100 }}>
          <section style={{ ...panelStyle, minHeight: 146, padding: 26 }}>
